...
package org.opendaylight.didm.vendor.noviflow;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.didm.vendor.noviflow.rev150211.NoviflowDeviceType;
...
/**
 * The noviflow OF driver does the following:
 *
 * 1. listen for node added/removed in inventory (future: filtered by device
 * type) 2. when a noviflow node is added, register the routed RPCs (other driver
 * types may register as DCLs for a feature such as vlan) 3. when a noviflow node
...
public class OpenFlowDeviceDriver implements OpenflowFeatureService, DataChangeListener, AutoCloseable {
	...
	public OpenFlowDeviceDriver(DataBroker dataBroker, RpcProviderRegistry rpcRegistry) {
		...
		if (this.salFlowService != null && this.salGroupService != null) {
			LOG.info("Noviflow driver correctly obtained the salflow and salgroup service.");
		}
	...
	public Future<RpcResult<AdjustFlowOutput>> adjustFlow(AdjustFlowInput input) {
		LOG.error("Noviflow adjustFlow");
		// Since noviflow is s/w based it should support all capabilities, just
		// echo the input flow
	...
	public void onDataChanged(AsyncDataChangeEvent<InstanceIdentifier<?>, DataObject> change) {
		...
		if (createdData != null) {
			for (Map.Entry<InstanceIdentifier<?>, DataObject> entry : createdData.entrySet()) {
				...
				if (isNoviflowDeviceType(deviceType.getDeviceType())) {
	...
	private static boolean isNoviflowDeviceType(Class<? extends DeviceTypeBase> deviceType) {
